	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				/*.accept() - позволяет серверу следить за портом, 
				 * или иначе говоря ждать подключения клиента. 
				 * Как только клиент подключается - сокет для клиента сразу же создается. 
				 * В противном случае выбрасывается исключение IOException.*/
